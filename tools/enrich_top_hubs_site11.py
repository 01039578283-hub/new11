"""Enrich only the 15 national/topic coaching hubs; never regenerate locality copy."""
...
def choose_centers(rel, examples):
    import re
    slug = rel.split('/')[-2]
    if rel in {'전국학원/index.html','과목별학원/index.html'}: slug = '영수학원'
...
def after_markup(copy, centers):
    ...
    return centers_markup(copy, centers) + '<section class="hub-guide" data-hub-enrichment="consultation" id="hub-consultation-guide"><div class="hub-wrap"><div class="hub-guide-heading"><p class="eyebrow">CONSULTATION NOTE</p><h2>상담 전에 학생의 학습 장면을 정리해 보세요</h2><p>학원에서 바꾸고 싶은 점을 정한 뒤, 학생이 실제로 공부한 자료와 생활 속 시간을 함께 살펴보면 상담 질문이 구체적이 됩니다.</p></div><div class="hub-guide-grid"><article class="hub-guide-card"><h3>정답을 보기 전의 생각</h3><p>풀이를 고치기 전 어디에서 멈췄는지 짧게 적어 두세요. 문제를 처음 읽었을 때의 이해와 설명을 들은 뒤의 이해를 나누면, 다음 수업에서 확인할 부분을 찾기 쉽습니다.</p></article><article class="hub-guide-card"><h3>집에서 이어지는 공부</h3><p>계획한 공부와 실제로 마친 공부를 나란히 적어 보세요. 이동 시간과 다른 활동을 고려해 혼자 복습할 수 있는 시간, 도움이 필요한 시간을 구별하는 자료로 활용할 수 있습니다.</p></article><article class="hub-guide-card"><h3>지점에 물어볼 내용</h3><p>방문할 곳의 주소와 희망 학년·과목을 먼저 확인하세요. 최근 학교 자료를 보여 주며 진도와 과제의 조정 방식, 질문과 피드백 방법, 수업 요일 및 교습비·교재비 포함 여부를 질문해 보세요.</p></article></div><div class="hub-guide-reading"><a href="/학습가이드/">학습관리 기준 읽기</a><a href="/과목별학원/">과목·학년별 안내 보기</a><a href="/상담문의/">상담 방법 확인하기</a></div></div></section>' + f'<section class="hub-guide" data-hub-enrichment="photos" id="hub-learning-space"><div class="hub-wrap"><div class="hub-guide-heading"><p class="eyebrow">LEARNING SPACE</p><h2>학습 공간 살펴보기</h2><p>와와 학습 공간의 공통 예시입니다. 실제 책상 배치와 시설은 지점별로 다를 수 있습니다.</p></div><div class="hub-guide-gallery">{photos}</div></div></section>'
...
def update_schema(soup, canonical, copy, centers, qa, modified, rel):
    ...
    graphs += [
        {'@type':'WebSite','@id':DOMAIN+'/#website','url':DOMAIN+'/',
         'name':'온담학습.com','publisher':{'@id':DOMAIN+'/#organization'}},
        {'@type':'Organization','@id':DOMAIN+'/#organization',
         'name':'온담학습','url':DOMAIN+'/'}
    ]
...
def update_page(rel, copy, examples, modified):
    ...
    if rel == '전국학원/index.html':
        ...
        directory.select_one('.section-head h2').string = '과목을 고른 뒤 가까운 동네를 찾아보세요'
        directory.select_one('.section-head .lead').string = '영어·수학을 각각 살펴보거나 두 과목의 학습 시간을 함께 점검할 수 있습니다.'
    ...
    if directory.select('.region-block'):
    ...
    if not soup.select_one('link[type="application/rss+xml"]'):
        soup.head.append(soup.new_tag('link',rel='alternate',type='application/rss+xml',title='온담학습 허브 소식',href=DOMAIN+'/rss.xml'))
